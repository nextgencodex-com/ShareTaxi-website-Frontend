...
export const useRideBooking = () => {
  ...
  // Get nearby drivers
  const findNearbyDrivers = useCallback(async (location, radius = 10) => {
    setLoading(true);
    try {
  ...
  // Track ride status
  const trackRide = useCallback(async (rideId) => {
    try {
...
export const useDriverActions = () => {
  ...
  // Register as driver
  const registerDriver = useCallback(async (driverData) => {
    setLoading(true);
    try {
  ...
  // Get pending rides
  const getPendingRides = useCallback(async (location, radius = 10) => {
    setLoading(true);
    try {
  ...
  // Accept a ride
  const acceptRide = useCallback(async (rideId, driverData) => {
    setLoading(true);
    try {
